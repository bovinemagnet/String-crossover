...
### Challenge Mode
Players must solve puzzles under constraints, for example:
- Maximum 3 crossings
...
- 2 Stars: Completed with minor inefficiencies
- 1 Star: Puzzle completed
...
`Score = Base Points + Efficiency Bonus + Time Bonus - Penalties`
...
### String Types
- Standard: Normal string
- Elastic: Can stretch around obstacles
- Metallic: Cannot cross other strings
- Magical: Can pass through designated barriers

### Layer System
Advanced puzzles may contain multiple routing layers.
Players switch between layers to route strings, creating strategic puzzles.
...
## Godot Fit
This design fits particularly well with Godot because string rendering can be implemented with `Line2D`, while the puzzle graph can be represented as a lightweight node network with JSON-defined levels.
